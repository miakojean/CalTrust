  ...
  <div class="my__rating_container">
    ...
    >
      <span class="my__star" v-for="i in fullStars" :key="'filled-' + i">★</span>
        <span 
        v-if="partialStarWidth > 0" 
        class="my__star partial__star"
        :style="{ background: `linear-gradient(90deg, var(--primary-color) ${partialStarWidth}%, #f3f3f3 ${partialStarWidth}%)` }"
        >
        ★
        </span>
...
.partial__star {
  color: rgb(88, 88, 88); /* étoile en blanc */
  background: linear-gradient(
    90deg,
    var(--primary-color) var(--partialStarWidth),
    gray var(--partialStarWidth)
  );
}
